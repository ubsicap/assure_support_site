	...
	class q2apro_userinfo_page {
		...
		function process_request($request) {
			...
			if( $transferString !== null ) {
				...
				$acceptanceString = '';
				if($aCount>0) {
					$acceptanceString = ' | &Oslash; ' . number_format( 100 * $answersBest / $aCount, 2, ',', '.') . ' %';
				}
				...
				$output .= '<p style="line-height:180%;">'.qa_lang('q2apro_userinfo_lang/member_since').' '.$accountAge.' '.$langDays.'<br />';
				...
				$output .= qa_lang('q2apro_userinfo_lang/received').': '.$receivedUpvotes.$receivedDownvotes.'</p>';
				...
				$output .= '<br />'.qa_lang('q2apro_userinfo_lang/answers').': '.$aCount.' | '.qa_lang('q2apro_userinfo_lang/best').': '.$answersBest . $acceptanceString;
				
				// output of badges, needs badges plugin
				$output .= (function_exists('qa_badge_plugin_user_widget') ? '<br/>'.qa_badge_plugin_user_widget($handle) : '');
