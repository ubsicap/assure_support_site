...
class qa_hashtagger
{
    ...
    private static function build_user_link($match)
    {
        ...
        if ($userid) {
            self::$userids[] = $userid;
            $url = qa_path_html("user/{$match['name']}", $rooturl = qa_opt('site_url'));
    ...
    private function init_filter(&$row, $type)
    {
        ...
        if ($convert_hashtags) {
            $row['content'] = $this->preg_call('%(?<!([^<]*<[^>]*))#(?P<word>[\w\-]+)%us', 'build_tag_link', $row['content']);
        }

        // Convert usernames, orig expression: %@(?P<name>[\w\-]+)%u
        if ($convert_usernames) {
            $row['content'] = $this->preg_call('%(?<!([^<]*<[^>]*))@(?P<name>[\w\-]+)%us', 'build_user_link', $row['content']);
        }
